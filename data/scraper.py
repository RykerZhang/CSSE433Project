...
def get_stats(soup):
    ...
    # data_abilities = "%"
    # for ab in soup.find("th", text="Abilities").next_sibling.next_sibling.find_all("a"):
    #     data_abilities = data_abilities + ab.text + "%"

    data_abilities = []
    for ab in soup.find("th", text="Abilities").next_sibling.next_sibling.find_all("a"):
        data_abilities.append(ab.text)

    first_ability = second_ability = hidden_ability = "Nan"

    first_ability = data_abilities[0]
    if (len(data_abilities) == 2):
        hidden_ability = data_abilities[1]
    if (len(data_abilities) == 3):
        second_ability = data_abilities[1]
        hidden_ability = data_abilities[2]

    training_catch_rate = soup.find(
        "th", text="Catch rate").next_sibling.next_sibling.text.split()[0]
    training_base_exp = soup.find(
        "th", text="Base Exp.").next_sibling.next_sibling.text
    training_growth_rate = soup.find(
        "th", text="Growth Rate").next_sibling.next_sibling.text
    breeding_gender = soup.find(
        "th", text="Gender").next_sibling.next_sibling.text
    if breeding_gender == "Genderless":
        breeding_gender = "Genderless,Genderless"
    stats_hp = soup.find("th", text="HP").next_sibling.next_sibling.text
    stats_attack = soup.find(
        "th", text="Attack").next_sibling.next_sibling.text
    stats_defense = soup.find(
        "th", text="Defense").next_sibling.next_sibling.text
    stats_sp_atk = soup.find(
        "th", text="Sp. Atk").next_sibling.next_sibling.text
    stats_sp_def = soup.find(
    ...
    return data_species, data_height, data_weight, first_ability, second_ability, hidden_ability, training_catch_rate, training_base_exp, \
...
def parse_infocards(soup, pokemon_infos):
    ...
    with open('pokemon_db1.csv', 'w') as f:
        f.write(pokemon_infos)

    # write infos line by line
    with open('pokemon_db1.csv', 'a') as f:
        for ic in infocards:
...
def main():
    ...
    pokemon_infos = """id_nb,name,type_1,type_2,link,data_species,data_height,data_weight,first_ability,second_ability,hidden_ability,training_catch_rate,training_base_exp,training_growth_rate,breeding_gender_male,breeding_gender_female,stats_hp,stats_attack,stats_defense,stats_sp_atk,stats_sp_def,stats_speed,stats_total,"""
